...
import { isOrderMatchingRider, findRiderByIdentifier, getCachedDeliveryRiders, setCachedDeliveryRiders, areDigitsMatching } from '../utils/partnerUtils';
                ...
                <div className="flex-1 p-4 sm:p-5 flex flex-col justify-between bg-gradient-to-b from-blue-950 via-slate-900 to-slate-950 text-white overflow-y-auto">
                  ...
                  <form 
                    onSubmit={async (e) => {
                      ...
                      // 1. Gather all potential riders from multiple sources (props, cache, initial mocks)
                      const allKnownRiders: DeliveryRider[] = [
                        ...riders,
                        ...getCachedDeliveryRiders(),
                        ...INITIAL_RIDERS
                      ];

                      // Deduplicate by ID
                      const uniqueRidersMap = new Map<string, DeliveryRider>();
                      allKnownRiders.forEach(r => {
                        if (r && r.id && !uniqueRidersMap.has(r.id)) {
                          uniqueRidersMap.set(r.id, r);
                        }
                      });
                      const combinedRidersList = Array.from(uniqueRidersMap.values());

                      // 2. Perform deep flexible matching (phone, digits, deviceNumber, name, CPF, or ID)
                      let matched = findRiderByIdentifier(combinedRidersList, inputVal);

                      // If not found yet, perform custom fuzzy matching for Brazilian phone formats
                      if (!matched) {
                        const cleanInputDigits = inputVal.replace(/\D/g, '');
                        matched = combinedRidersList.find(r => {
                          if (r.deviceNumber && r.deviceNumber.trim().toLowerCase() === inputVal.toLowerCase()) return true;
                          if (r.deviceNumber && cleanInputDigits && areDigitsMatching(r.deviceNumber, cleanInputDigits)) return true;
                          if (r.phone && cleanInputDigits && areDigitsMatching(r.phone, cleanInputDigits)) return true;
                          if (r.cpfCnpj && cleanInputDigits && areDigitsMatching(r.cpfCnpj, cleanInputDigits)) return true;
                          if (r.name && cleanInputDigits.length >= 8 && r.name.replace(/\D/g, '').includes(cleanInputDigits.slice(-8))) return true;
                          if (r.name && r.name.toLowerCase().includes(inputVal.toLowerCase())) return true;
                          return false;
                        });
                      }

                      // If STILL not found and input is a valid 8-11 digit phone/device (like 11970791804), create or fallback seamlessly
                      if (!matched) {
                        ...
                        if (cleanDigits.length >= 8) {
                          const formattedPhone = cleanDigits.length === 11 
                            ...
                            : (cleanDigits.length === 10 ? `(${cleanDigits.slice(0, 2)}) ${cleanDigits.slice(2, 6)}-${cleanDigits.slice(6)}` : inputVal);
                          
                          matched = {
                            id: `ent-${cleanDigits.slice(-8)}`,
                            name: `Condutor (${formattedPhone})`,
                            avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&q=80',
                            vehicle: 'Moto',
                            rating: 5.0,
                            status: 'Em rota',
                            phone: formattedPhone,
                            deviceNumber: cleanDigits,
                            password: '1234',
                            ...
                            realGeoLng: -46.70118,
                            lastGpsUpdate: 'Agora mesmo'
                          };
                          setCachedDeliveryRiders([matched, ...combinedRidersList]);
                        }
                      }
                      ...
                      // Accepts matched.password, '1234', '123456', 'admin', 'vinimap', '12345'
